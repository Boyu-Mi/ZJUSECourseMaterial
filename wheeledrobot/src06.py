    ...
    def on_press(self, event):
        ...
        if(event.key == ' '):
            ...
            if self.planning_target is not None and self.planner is not None:
                print("do planning...")
                px, py = planner.planning(self.planning_obs[:, 0], self.planning_obs[:, 1], self.x,
                                          self.y, self.planning_target[0], self.planning_target[1], -10, -10, 10, 10)
...
if __name__ == "__main__":
    ...
    planner = AStarPlanner(
        0.2, Playground.planning_obs_radius+DWAConfig.robot_radius)
    vplanner = dwa()
